...
var require_logform = __commonJS({
  "node_modules/logform/index.js"(exports) {
    "use strict";
    var format = exports.format = require_format2();
...
${stack}`;
  ...
  transports: [
    new import_winston.default.transports.File({ filename: logFile })
  ]
});
function logSessionStart() {
  logger.info("=".repeat(50));
  logger.info(`Starting Positron Dev Tools MCP Server at ${(/* @__PURE__ */ new Date()).toISOString()}`);
  logger.info("=".repeat(50));
}
...
var ProcessManager = class {
   ...
   */
  addToBuffer(buffer, line, maxLines) {
    buffer.push(line);
   ...
   */
  async startDaemon(name, command, cwd) {
...
${errorOutput}`
      ...
      runningProcess.state = "running" /* RUNNING */;
      child.on("exit", (code, signal) => {
        logger.info(`Daemon ${name} exited with code ${code} and signal ${signal}`);
        const proc = this.runningProcesses.get(name);
        if (proc) {
          proc.state = code === 0 ? "stopped" /* STOPPED */ : "failed" /* FAILED */;
          setTimeout(() => this.runningProcesses.delete(name), 5e3);
        }
      });
      child.on("error", (err) => {
   ...
   */
  isDaemonRunning(name) {
    const proc = this.runningProcesses.get(name);
    return proc !== void 0 && proc.state === "running" /* RUNNING */;
  }
  /**
  ...
  getProcessStatus(name) {
    ...
    const uptime = Math.floor((Date.now() - proc.startTime.getTime()) / 1e3);
    const recentOutput = [
...
var ToolExecutor = class {
   ...
   */
  async executeTool(toolConfig, toolName, args) {
    if (!toolConfig) {
...
`;
   ...
   * Get logs from a specific daemon
   */
  getProcessLogs(daemonName, lines) {
...
${output}`;
  ...
  getDaemonStatus() {
    ...
    for (const daemon of daemons) {
      const runtime = Math.floor((Date.now() - daemon.startTime.getTime()) / 1e3);
...
var SimpleCommandsServer = class {
  server;
  config;
  projectRoot;
  toolExecutor;
  ...
  setupHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      const isAutoGenerated = !toolConfig && (name.endsWith("_status") || name.endsWith("_stop") || name.endsWith("_logs") || name.endsWith("_start"));
  ...
  async loadConfig() {
    const configPath = process.env.MCP_CONFIG_PATH || path2.join(__dirname2, "..", "config.json");
    try {
